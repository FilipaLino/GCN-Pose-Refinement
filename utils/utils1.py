...
def print_error_action(action_error_sum, show_protocol2=False):
    mean_error_each = {'p1': 0.0, 'p2': 0.0, 'p3':0.0, 'p4':0.0}
    mean_error_all = {'p1': AccumLoss(), 'p2': AccumLoss(), 'p3': AccumLoss(), 'p4': AccumLoss()}
    if show_protocol2 or show_protocol3 or show_protocol4 :
        print("{0:=^12} {1:=^10} {2:=^8} {3:=^8} {4:=^8}".format("Action", "p#1 mm", "p#2 mm", "p#3 mm", "p#4 mm"))
        for action,value in action_error_sum.items():
            ...
            print("{0:>6.2f} {1:>10.2f} {2:>10.2f} {3:>10.2f}".format(mean_error_each['p1'],mean_error_each['p2'],mean_error_each['p3'],mean_error_each['p4']))
        print("{0:<12} {1:>6.2f} {2:>10.2f} {3:>10.2f} {4:>10.2f}".format("Average", mean_error_all['p1'].avg, mean_error_all['p2'].avg, mean_error_all['p3'].avg, mean_error_all['p4'].avg))
    else:
...
def define_frame_error_list(actions):
    """
    ...
    """
    error_sum = {}
    error_sum.update({actions[i]: [] for i in range(len(actions))})
    return error_sum
...
def define_error_list(actions):
    ...
    """
    error_sum = {}
    error_sum.update({actions[i]: {'p1':AccumLoss(), 'p2':AccumLoss(),  'p3':AccumLoss(),  'p4':AccumLoss()} for i in range(len(actions))})
    return error_sum
