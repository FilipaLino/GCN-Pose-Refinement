...
h36m_skeleton = Skeleton(parents=[-1, 0,  1,  2,  0,  4,  5,  0,  7,  8,  9,  7, 11, 12,  7, 14, 15],
    ...
    def __init__(self, path, opt,remove_static_joints=True):
        ...
        for subject, actions in data.items():
            ...
            if subject[0] == 'S':
                ...
                synth_subject =  subject[1:] 
                file_path = path_synth + '/' + synth_subject + '/Cameras/camera_params_'+ synth_subject+'.npz'

                # Load serialized dataset
                data_int = np.load(file_path, allow_pickle=True)['intrinsic_params']
                data_ext = np.load(file_path, allow_pickle=True)['extrinsic_params'].item()[synth_subject]
                ...
                for camera in data_int:
                    camera_int_params = {
                        'id': cam_map1[camera['id']] if subject == 'SS1' else cam_map2[camera['id']],
                ...
                for i, cam_ext in enumerate(cameras_extrinsic_params):
                    cam = copy.deepcopy(cameras_intrinsic_params[i])
                    cam.update(cam_ext)
                    for k, v in cam.items():
                        if k not in ['id', 'res_w', 'res_h']:
                            cam[k] = np.array(v, dtype='float32')
                    ...
                    cam['intrinsic'] = np.concatenate((cam['focal_length'],
                                                       ...
                                                       cam['radial_distortion'],
                                                       cam['tangential_distortion']))
                    self._cameras[subject].append(cam)
