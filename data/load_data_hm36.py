...
class Fusion(data.Dataset):
    ...
    def prepare_data(self, dataset, folder_list):
        ...
        print('Loading 3D predictions...')
        keypoints = np.load(self.root_path + 'data3d_predictions' + '.npz',allow_pickle=True)
            ...
            for action in dataset[subject].keys():
                ...
                for cam_idx in range(len(keypoints[subject][action])):
                    ...
                    if keypoints[subject][action][cam_idx].shape[0] < mocap_length: 
                        #from VideoPose3D input video without subject
                        nan_frames = mocap_length - keypoints[subject][action][cam_idx].shape[0] 
                        dataset[subject][action]['positions_3d'][cam_idx]=dataset[subject][action]['positions_3d'][cam_idx][nan_frames:]
                        mocap_length = dataset[subject][action]['positions_3d'][cam_idx].shape[0]
